...
import com.coship.hadoop.hdfs.server.socketspeed.SocketSpeed;
import com.coship.hadoop.hdfs.server.system.SystemActivity;
import com.coship.hadoop.hdfs.server.system.SystemInfo;
...
public class DataNode extends Configured implements InterDatanodeProtocol,
	...
	void startDataNode(Configuration conf, AbstractList<File> dataDirs,
		...
		if (simulatedFSDataset) {
			...
			/*
			 * @author	coship	cofs
			 * create SystemInfo , SystemActivity ,SocketSpeed
			 * 
			 * for register MBean
			 * 
			 * */
			SystemInfo sysInfo = new SystemInfo();	//new core
			SystemActivity systemActivity = new SystemActivity(conf,this); // new core
			SocketSpeed socketSpeed = new SocketSpeed(conf); // new core
		}
		...
		this.infoServer = (secureResources == null) ? new HttpServer(
				...
				: new HttpServer("datanode", infoHost, tmpInfoPort,
						tmpInfoPort == 0, conf, SecurityUtil.getAdminAcls(conf,
								DFSConfigKeys.DFS_ADMIN), secureResources
								.getListener());
		...
		myMetrics = DataNodeInstrumentation.create(conf, dnRegistration
				.getStorageID());
			...
			return loginUgi
					.doAs(new PrivilegedExceptionAction<InterDatanodeProtocol>() {
						public InterDatanodeProtocol run() throws IOException {
							return (InterDatanodeProtocol) RPC
									.getProxy(InterDatanodeProtocol.class,
											InterDatanodeProtocol.versionID,
											addr, conf);
						}
					});
		} catch (InterruptedException ie) {
	...
	private void register() throws IOException {
		...
		if (!isBlockTokenInitialized) {
			...
			if (isBlockTokenEnabled) {
				...
				LOG
						.info("Block token params received from NN: keyUpdateInterval="
	...
	private void handleDiskError(String errMsgr) {
		...
		LOG
				.warn("DataNode.handleDiskError: Keep Running: "
						+ hasEnoughResource);
		...
		while (shouldRun) {
			try {
				...
				if (startTime - lastBlockReport > blockReportInterval) {
					...
					// Log the block report processing stats from Datanode
					// perspective
					long brSendCost = now() - brSendStartTime;
					long brCreateCost = brSendStartTime - brCreateStartTime;
					myMetrics.addBlockReport(brSendCost);
					LOG.info("BlockReport of " + bReport.length
							+ " blocks took " + brCreateCost
	...
	private LocatedBlock syncBlock(Block block, List<BlockRecord> syncList,
		...
		if (syncList.isEmpty()) {
			...
			if (isBlockTokenEnabled) {
				b.setBlockToken(blockTokenSecretManager.generateToken(null, b
						.getBlock(), EnumSet
						.of(BlockTokenSecretManager.AccessMode.WRITE)));
			}
			return b;
	...
	public DataNodeInstrumentation getMyMetrics() {
		return myMetrics;
	}
